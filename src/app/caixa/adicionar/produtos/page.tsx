...
export default function ProductSelectionPage() {
  ...
  >([]);
  const selected = useIncomeFormStore((s) => s.selectedProducts);
  const setSelectedProducts = useIncomeFormStore((s) => s.setSelectedProducts);
  ...
  // Selection is persisted via Zustand; no manual localStorage hydration needed
  ...
  const addItem = (productId: number, unitPrice: number) => {
    ...
    setSelectedProducts({
      ...prev,
      [productId]: { quantity: next, unitPrice },
    });
  };
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    selectedData.quantity > 0 ? "bg-slate-50" : "",
                  )}
                >
                  <div className="flex flex-col">
                    <span className="font-medium">{p.name}</span>
                    <span className="text-xs text-slate-500">
                      Preço {formatCurrency(Number(p.price))} • Estoque{" "}
                      {available}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      type="button"
                      size="icon"
                      onClick={() => addItem(p.id, selectedData.unitPrice)}
                      disabled={!canAdd}
