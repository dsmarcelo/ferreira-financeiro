...
function groupByDate(purchases: ProductPurchase[]) {
  return purchases.reduce<Record<string, ProductPurchase[]>>((acc, purchase) => {
          ...
          {(grouped[date]?.slice(1) ?? []).map((purchase) => (
            <EditProductPurchase data={purchase} key={purchase.id}>
              <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium pl-12">
                <span className="flex-1 font-normal text-black/80">{purchase.description}</span>
                <span className="font-medium text-right min-w-[80px]">{formatCurrency(purchase.value)}</span>
              </div>
            </EditProductPurchase>
          ))}
        </div>
